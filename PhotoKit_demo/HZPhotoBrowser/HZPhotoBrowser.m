...
//    view.beginLoadingImage = YES;
    ...
    if ([self highQualityImageForIndex:index]) {
        view.imageview.image = [self highQualityImageForIndex:index];
    } else {
        view.imageview.image = [self placeholderImageForIndex:index];
    }
...
//#pragma mark 加载本地图片
//- (void)setupLocalImageForIndex:(NSInteger)index {
//    HZPhotoBrowserView *view = _scrollView.subviews[index];
//    if (view.beginLoadingImage) return;
//    if ([self highQualityImageForIndex:index]) {
//        view.imageview.image = [self highQualityImageForIndex:index];
...
- (void)getImages:(callBackImages)callback in:(NSInteger)index {
    if ([_delegate respondsToSelector:@selector(photoBrowserImages:index:)]) {
        [_delegate photoBrowserImages:^(NSArray *images, NSArray *thumls) {
            callback(images, thumls);
        } index:index];
    }
}
